...
-- === MONEY FARM V2 (clean state machine) ===
local MoneyV2 = { state = "idle" }
local moneyV2Conn

do
	local function install()
		local SCAN_GAP = 15
		local DROUGHT_LIMIT = 18
		local MAX_SWINGS = 150
		local MAX_DEPLETED = 60
		local SWING_BURST = 5
		local SWING_SINK = 1.2

		local state = "idle"
		local swingClock = 0
		local noCrystalSince = 0
		local totalSwings = 0
		local depleted = {}
		local depCount = 0
		local scanAngle = 0
		local lastPickCount = 0
		local curX, curZ, curY

		local function cellKey(x, z) return math.floor(x / 15) .. "," .. math.floor(z / 15) end

		local function terrainExists(x, z)
			local hit = Workspace:Raycast(Vector3.new(x, 2000, z), Vector3.new(0, -3000, 0))
			return hit and hit.Position.Y > 6 and hit.Position.Y or nil
		end

		local function scanTarget()
			local center = mountainSpot()
			if not center then return nil end
			for dist = 15, 200, 25 do
				for off = 0, 5 do
					local a = math.rad(scanAngle + off * 60)
					local x = center.X + math.cos(a) * dist
					local z = center.Z + math.sin(a) * dist
					if not depleted[cellKey(x, z)] then
						local y = terrainExists(x, z)
						if y then return x, z, y end
					end
				end
			end
			return nil
		...
		local function equipPick()
			...
			local function consider(t)
				...
				local p = tonumber(getAttr(t, "DigPower")) or 0
				if p >= bestP then best = t; bestP = p end
			end
			consider(char:FindFirstChildOfClass("Tool"))
			if bp then for _, t in ipairs(bp:GetChildren()) do consider(t) end end
			if best and best ~= char:FindFirstChildOfClass("Tool") then
				pcall(function() hum:EquipTool(best) end)
			end
			return best
		end

		local function doSwing(x, y, z)
			local ev = Farm.digEvent()
			local held = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Tool")
			if not ev or not held then return end
			local name = held.Name
			for i = 0, SWING_BURST - 1 do
				ev:FireServer(name, Vector3.new(x, y - i * SWING_SINK, z))
			end
		end
		...
		local function step(dt)
			local now = os.clock()
			local root = getRoot()
			if not root or state == "idle" then return end

			-- Sell if backpack full (reuse existing doSell)
			local free = backpackFree()
			if free <= 0 then
				if doSell() then task.wait(2); return end
			end

			pickupStep()

			local held = equipPick()
			if not held then return end

			if state == "scan" then
				scanAngle = scanAngle + 37
				local x, z, y = scanTarget()
				if x then
					curX, curZ, curY = x, z, y
					state = "move"
					noCrystalSince = now
					totalSwings = 0
				else
					depCount += 1
					if depCount >= 5 then
						Library:Notify("MoneyV2: mountain depleted, hopping", 3)
						Net.hop(); stopAll(); return
					end
					local c = mountainSpot() or root.Position
					teleportTo(c + Vector3.new(math.random(-100, 100), 30, math.random(-100, 100)))
					task.wait(0.5)
				end
				return
			end
			...
			if state == "move" then
				local dist = Vector3.new(curX, 0, curZ) - Vector3.new(root.Position.X, 0, root.Position.Z)
				if dist.Magnitude > 10 then
					local pos = Vector3.new(curX, curY + 8, curZ)
					teleportTo(pos); requestStream(pos)
			...
			if state == "mine" then
				if not curX then state = "scan"; return end

				-- Drought: no crystal picked up for DROUGHT_LIMIT seconds
				local noCrystalTime = now - noCrystalSince
				if totalSwings > 3 and noCrystalTime >= DROUGHT_LIMIT then
					depleted[cellKey(curX, curZ)] = true
					depCount += 1
					if depCount >= MAX_DEPLETED then
						Library:Notify("MoneyV2: all cells depleted, hopping", 3)
						Net.hop(); stopAll(); return
					end
					curX = nil; state = "scan"
					return
				...
				if totalSwings >= MAX_SWINGS then
					depleted[cellKey(curX, curZ)] = true
					curX = nil; state = "scan"
					return
				end
				...
				-- Hover & swing
				local pos = Vector3.new(curX, curY + 7, curZ)
				if (root.Position - pos).Magnitude > 5 then
					holdAt(CFrame.new(pos, pos - Vector3.new(0, 1, 0)))
				end

				swingClock += dt
				local gap = math.max(0.02, (tonumber(getAttr(held, "DigSpeed")) or 0.04) * 0.4)
				if swingClock >= gap then
					swingClock = 0; totalSwings += 1
					doSwing(curX, curY, curZ)
				end

				-- Track pickup via EXT.picked counter
				if EXT.picked > lastPickCount then
					lastPickCount = EXT.picked
					noCrystalSince = now
				end
			end
		end
		...
		-- Public
		function MoneyV2.setActive(v)
			if not v then stopAll(); return end
			state = "scan"; scanAngle = math.random(0, 360)
			depCount = 0; curX = nil; noCrystalSince = os.clock()
			totalSwings = 0; swingClock = 0
			Library:Notify("Money Farm V2 started", 2)
		end

		moneyV2Conn = RunService.Heartbeat:Connect(function(dt)
			if state ~= "idle" then
				pcall(step, dt)
			end
		end)
		...
		-- GUI
		local box = Tabs.farming:AddRightGroupbox("Money Farm V2", "zap")
		box:AddToggle("MoneyV2Toggle", {
			Text = "Auto Farm V2",
			Default = false,
			Callback = MoneyV2.setActive,
		})
		box:AddLabel("Clean state machine. Scans mountain in spiral, mines each spot until 18s drought or 150 swings, then moves on. Hops when fully depleted.", true)
	end
